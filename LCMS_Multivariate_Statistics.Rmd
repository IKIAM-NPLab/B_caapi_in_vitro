...
data <- read_from_excel(file = "Data/BC_LCMS_data_to_notame.xlsx",
                        sheet = 1, corner_row = 5, corner_column = "I",
                        rt_limits = c(0, 60), split_by = c("Column", "Ion Mode"))
...
clustered <- cluster_features(corrected_no_blank,
                              ...
                              d_thresh = 0.85,
                              plotting = F,
                              prefix = paste0(ppath, "Result/ClusterLC/")
                              )

#clustered@featureData@data$Flag[clustered@featureData@data$Metabolite %in% c("Leptaflorine", "Harmaline")] <- NA

#clustered@featureData@data$Cluster_ID[clustered@featureData@data$Metabolite %in% c("Leptaflorine")] <- "Rtx5_EI_216_095157131654a34_8623"

#clustered@featureData@data$Cluster_ID[clustered@featureData@data$Metabolite %in% c("Harmaline")] <- "Rtx5_EI_213_057266311087a36_368797"

#clustered@featureData@data$Cluster_size[clustered@featureData@data$Metabolite %in% c("Leptaflorine", "Harmaline")] <- "1"

#clustered@featureData@data$Cluster_features[clustered@featureData@data$Metabolite %in% c("Leptaflorine")] <- "Rtx5_EI_216_095157131654a34_8623"

#clustered@featureData@data$Cluster_features[clustered@featureData@data$Metabolite %in% c("Harmaline")] <- "Rtx5_EI_213_057266311087a36_368797"


#clustered2 <- construct_metabosets(
 # exprs = clustered@assayData$exprs,
  #pheno_data = clustered@phenoData@data,
  #feature_data = clustered@featureData@data,
#  group_col = "Group"
#)
...
save(compressed, file = paste0(ppath, "Result/Notame_LC-MS_out.RData"))
...
mode_mass_norm <- modes_mass_norm$as_bb
...
figure_1a <- ggplot(scores,
  ...
  theme(legend.position = c(0.915, 0.190),
        legend.background = element_rect(fill = "white", color = "black")) +
...
# explant legend
lgd2 <- Legend(labels = gt_render(c("*organogenic_meristem*",
                                    "*stem_explant*",
                                    "*non-organogenic_meristems*",
                                    "*leaf_explant*",
                                    "*Callus*")),
