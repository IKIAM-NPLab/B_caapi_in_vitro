...
figure_1a <- ggplot(scores, aes(PC1, PC2, shape = Group, color = Group)) +
  ...
  geom_point(size = 2) +
  guides(x = guide_axis(title = paste0("PC1 (", round(summary(is_norm_pca)$importance[2,1]*100, 2), " %)")),
         y = guide_axis(title = paste0("PC2 (", round(summary(is_norm_pca)$importance[2,2]*100, 2), " %)"))) +
  theme_classic() +
...
homoscedasticity_test_is <- perform_homoscedasticity_tests(
  mode_is_norm,formula_char = "Feature~ Group")

```
```{r echo=TRUE, message=FALSE, warning=FALSE, error=FALSE}
exprs_is_nomr <- exprs(mode_is_norm)

shapiro_results_is_norm <- apply(exprs_is_nomr, 1, function(x) shapiro.test(x)$p.value)

normality_is <- data.frame(
  Feature_ID = rownames(exprs_is_nomr),
...
homoscedasticity_test_mass <- perform_homoscedasticity_tests(
  mode_mass_norm,formula_char = "Feature~ Group")
...
homoscedasticity_test_is_mass <- perform_homoscedasticity_tests(
  mode_is_mass_norm,formula_char = "Feature~ Group")

homoscedasticity_test_is_mass
