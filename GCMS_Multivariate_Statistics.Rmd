...
pqn_set <- pqn_normalization(mode_is,
                             ref = c("qc"),
...
merged <- merge_metabosets(imputed)

merged_no_qc <- drop_qcs(merged)

feat_table <- exprs(merged_no_qc)

masses <- merged_no_qc@phenoData@data$Mass
...
# Eliminar la columna del blanco (ID_4)
data_noBlank <- mass_norm[, colnames(mass_norm) != "ID_4"]

# Transponer para PCA 
mass_norm_t <- t(data_noBlank)
...
modes_mass <- construct_metabosets(
  exprs = mass_norm,
  pheno_data = merged_no_qc@phenoData@data,      
  feature_data = merged_no_qc@featureData@data,         
  group_col = "Group"                    
)

mode_mass <- modes_mass$Rtx5_EI

# Probabilistic quotient normalization
pqn_set <- pqn_normalization(mode_mass,
                             ref = c("all"),
                             method = c("median", "mean"),
                             all_features = FALSE)
