...
# Transform the internal standard-normalized data
tranfromed_is_norm <- log10(is_norm)

# Transpose feature matrix for PCA (samples as rows)
is_norm_t <- t(tranfromed_is_norm)
...
# PCA plot: PC1 vs PC2
figure_1b <- ggplot(scores, aes(PC1, PC2, shape = Group, color = Group)) +
  geom_point(size = 2) +
  labs(
    x = paste0("PC1 (", round(summary(is_norm_pca)$importance[2,1]*100, 2), " %)"),
    y = paste0("PC2 (", round(summary(is_norm_pca)$importance[2,2]*100, 2), " %)")
...
# Build metaboset from internal standard-normalized data
modes_is_norm_trans <- construct_metabosets(
  exprs = tranfromed_is_norm,
  pheno_data = imputed@phenoData@data,
  feature_data = imputed@featureData@data,
  group_col = "Group"
)
...
# Select specific mode (e.g., GC-MS method)
mode_is_norm_trans <- modes_is_norm_trans$Rtx5_EI

# Perform tests for homoscedasticity
homoscedasticity_test_is_trans <- perform_homoscedasticity_tests(
  mode_is_norm_trans,
  formula_char = "Feature ~ Group",
  all_features = TRUE
)
...
# Summary of results
total <- nrow(homoscedasticity_test_is_trans)
passed <- sum(homoscedasticity_test_is_trans$Fligner_P_FDR > 0.05)
failed <- sum(homoscedasticity_test_is_trans$Fligner_P_FDR <= 0.05)
...
# Extract expression matrix from the metaboset object
exprs_is_norm_trans <- exprs(mode_is_norm_trans)
...
tranfromed_mass_norm <- log10(mass_norm)

# Transpose feature matrix for PCA (samples as rows)
is_mass_t <- t(tranfromed_mass_norm)

# Perform PCA with scaling
is_mass_pca <- prcomp(is_mass_t, scale. = TRUE)

# Merge PCA scores with sample metadata
scores <- is_mass_pca$x %>%
  data.frame() %>%
  mutate(Sample_ID = rownames(.)) %>%
  left_join(imputed@phenoData@data, by = "Sample_ID")
...
figure_1b <- ggplot(scores, aes(PC1, PC2, shape = Group, color = Group)) +
  geom_point(size = 2) +
  labs(
    x = paste0("PC1 (", round(summary(is_mass_pca)$importance[2,1]*100, 2), " %)"),
    y = paste0("PC2 (", round(summary(is_mass_pca)$importance[2,2]*100, 2), " %)")
  ) +
  theme_classic() +
  theme(
    legend.position = c(-0.17, -0.24),
...
# Build metaboset from mass-normalized data
modes_mass_norm_trans <- construct_metabosets(
  exprs = tranfromed_mass_norm,
  pheno_data = imputed@phenoData@data,
  feature_data = imputed@featureData@data,
  group_col = "Group"
)
...
# Select the relevant mode (e.g., Rtx5_EI method)
mode_mass_norm_trans <- modes_mass_norm_trans$Rtx5_EI

# Perform tests for homoscedasticity
homoscedasticity_test_mass_trans <- perform_homoscedasticity_tests(
  mode_mass_norm_trans,
  formula_char = "Feature ~ Group",
    all_features = TRUE
)

# Summary of homoscedasticity results
total <- nrow(homoscedasticity_test_mass_trans)
passed <- sum(homoscedasticity_test_mass_trans$Fligner_P_FDR > 0.05)
failed <- sum(homoscedasticity_test_mass_trans$Fligner_P_FDR <= 0.05)
...
# Summary of normality results
total <- nrow(normality_mass_trans)
passed <- sum(normality_mass_trans$Shapiro_FDR > 0.05)
failed <- sum(normality_mass_trans$Shapiro_FDR <= 0.05)
...
tranfromed_is_mass_norm <- log10(is_mass_norm)

# Transpose feature matrix for PCA (samples as rows)
is_mass_t <- t(tranfromed_is_mass_norm)

# Perform PCA with scaling
trans_is_mass_pca <- prcomp(is_mass_t, scale. = TRUE)

# Merge PCA scores with sample metadata
scores <- trans_is_mass_pca$x %>%
  data.frame() %>%
  mutate(Sample_ID = rownames(.)) %>%
  left_join(imputed@phenoData@data, by = "Sample_ID")
...
figure_1b <- ggplot(scores, aes(PC1, PC2, shape = Group, color = Group)) +
  geom_point(size = 2) +
  labs(
    x = paste0("PC1 (", round(summary(trans_is_mass_pca)$importance[2,1]*100, 2), " %)"),
    y = paste0("PC2 (", round(summary(trans_is_mass_pca)$importance[2,2]*100, 2), " %)")
  ) +
  theme_classic() +
  theme(
    legend.position = c(-0.17, -0.24),
...
# Build metaboset using IS + Mass normalized data
modes_is_mass_norm_trans <- construct_metabosets(
  exprs = tranfromed_is_mass_norm,
  pheno_data = imputed@phenoData@data,
  feature_data = imputed@featureData@data,
  group_col = "Group"
)
...
# Select the analytical mode (e.g., Rtx5_EI)
mode_is_mass_norm_trans <- modes_is_mass_norm_trans$Rtx5_EI

# Perform Levene's test for homoscedasticity
homoscedasticity_test_is_mass_trans <- perform_homoscedasticity_tests(
  mode_is_mass_norm_trans,
  formula_char = "Feature ~ Group",
  all_features = TRUE
)

# Summarize results
total <- nrow(homoscedasticity_test_is_mass_trans)
passed <- sum(homoscedasticity_test_is_mass_trans$Fligner_P_FDR > 0.05)
failed <- sum(homoscedasticity_test_is_mass_trans$Fligner_P_FDR <= 0.05)
...
# Extract expression matrix from the IS + Mass normalized metaboset
exprs_is_mass_norm_trans <- exprs(mode_is_mass_norm_trans)

# Apply Shapiro-Wilk test to each feature (row-wise)
shapiro_results_is_mass_norm_trans <- apply(exprs_is_mass_norm_trans, 1, function(x) shapiro.test(x)$p.value)

# Compile results into a data frame with adjusted p-values (FDR)
normality_is_mass_norm <- data.frame(
  Feature_ID = rownames(exprs_is_mass_norm_trans),
  Shapiro_p = shapiro_results_is_mass_norm_trans,
  Shapiro_FDR = p.adjust(shapiro_results_is_mass_norm_trans, method = "fdr")
)
